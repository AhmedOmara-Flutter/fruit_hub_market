part of 'profile_cubit.dart';

@immutable
sealed class ProfileState {}

final class ProfileInitial extends ProfileState {}

final class ProfileUploadImageLoading extends ProfileState {}
final class ProfileUploadImageSuccess extends ProfileState {
  final String image;
  ProfileUploadImageSuccess({required this.image});

}
final class ProfileUploadImageError extends ProfileState {
  final String errMessage;
  ProfileUploadImageError({required this.errMessage});
}

final class ProfileGetImageLoading extends ProfileState {}
final class ProfileGetImageSuccess extends ProfileState {
  final String image;
  ProfileGetImageSuccess({required this.image});
}
final class ProfileGetImageError extends ProfileState {
  final String errMessage;
  ProfileGetImageError({required this.errMessage});
}
